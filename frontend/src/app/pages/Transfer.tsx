...
} from './transfer/accountNumberDetector.js'
...
const BANK_OPTIONS = TRANSFER_BANK_OPTIONS as BankOption[]

const REQUIRED_ACCOUNT_LENGTH = 10
const SOURCE_BANK = BANK_OPTIONS.find((bank) => bank.id === 'woori') ?? BANK_OPTIONS[0]
...
function BankMark({ bank, size = 'md' }: { bank: BankOption; size?: 'xs' | 'sm' | 'md' | 'lg' }) {
  const sizeClassName = {
    xs: 'h-4 w-4',
    sm: 'h-5 w-5',
    md: 'h-8 w-8',
...
export function Transfer() {
  ...
  const handleStartMemoEdit = (memoStep: Extract<TransferStep, 'editRecipientMemo' | 'editSenderMemo'>) => {
    setMemoDraft(memoStep === 'editRecipientMemo' ? recipientMemoName : senderMemoName)
    setStep(memoStep)
  }
  ...
  if (step === 'editRecipientMemo' || step === 'editSenderMemo') return renderMemoEditStep()

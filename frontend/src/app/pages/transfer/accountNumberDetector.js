...
export const TRANSFER_BANK_OPTIONS = BANK_PLUGINS_DATA.map(({ id, bankName }) => ({
  id,
  name: bankName,
}))
...
function evaluateRule(cleanNumber, rule) {
...
export class GenerateDetector {
  constructor(bankId, bankName, accountRules) {
    this.bankId = bankId
...
export class AccountNumberDetector {
  constructor(bankData = BANK_PLUGINS_DATA) {
...
export function detectAccountNumber(accountNumber) {

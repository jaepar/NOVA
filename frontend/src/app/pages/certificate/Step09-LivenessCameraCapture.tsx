...
import './Step09-LivenessCameraCapture.css'
...
const registeredImageBucket =
  (import.meta.env.VITE_LIVENESS_REGISTERED_IMAGE_BUCKET as string | undefined) ??
  'nova-object-bucket'
...
export function LivenessCameraCapture() {
  ...
  const navigateToStep08 = () => {
    navigate(STEP08_PATH, { replace: true, state: { preserveStep08State: true } })
  }
  ...
  const triggerSdkCancel = () => {
    const cancelButton =
      document.querySelector<HTMLButtonElement>('.amplify-liveness-cancel-button') ??
  ...
  useEffect(() => {
    ...
    mediaDevices.getUserMedia = (async (...args: Parameters<typeof originalGetUserMedia>) => {
  ...
  const handleRetry = () => {
    ...
    ;(async () => {
      try {
        ...
        setFailureMessage('재촬영 세션 생성에 실패했습니다. 잠시 후 다시 시도해 주세요.')
      } finally {
        setIsRetrying(false)
      }
    })()
  ...
  if (!hasAwsConfig) {
    return (
      <MobileLayout title="비대면 신원확인" backPath={STEP08_PATH}>
  ...
  return (
    ...
    >
      <div className="space-y-4 nova-liveness-surface" ref={detectorContainerRef}>
        ...
        {isDetectorVisible && isCameraActive && liveHintText && !failureMessage && (
          <div className="nova-liveness-guidance-toast" role="status" aria-live="polite">
            {liveHintText}
          </div>
        )}

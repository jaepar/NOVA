...
- Gateway: Java 17, Spring Boot, JPA, MySQL, HMAC-SHA256 기반 신원 조회
- FDS: Python, FastAPI, Isolation Forest 기반 이상거래 점수화
...
NOVA는 사용자 접근 계층은 클라우드에 두고, 금융 원장과 정부 신원 DB는 온프레미스 경계 안에 분리한 하이브리드 구조입니다. 사용자는 Vercel에 배포된 프론트엔드에 접속하고, Route 53, WAF, ALB를 거쳐 AWS private subnet의 Backend/AI 서버로 요청이 전달됩니다.

AWS 내부에서는 Backend와 AI 서버가 Multi-AZ로 배치되고, RDS는 Primary/Standby 구조와 RDS Proxy를 통해 장애 대응성을 확보합니다. Redis는 세션·락·멱등 처리에 사용되며, S3는 여권/외국인등록증 이미지와 구직 포트폴리오 파일 저장소로 사용됩니다. 개발자 접근은 Bastion Host를 통해 private subnet으로 제한됩니다.

온프레미스 영역은 OpenStack VM 기반으로 구성되며, Government DB, Core Banking Server, Core Banking DB, FDS Server, On-Premise Gateway가 배치됩니다. 클라우드 Backend는 Transit Gateway와 Site-to-Site VPN을 통해 On-Premise Gateway에 접근하고, Government DB나 Core Banking DB의 접속 정보를 직접 보유하지 않습니다. 이 구조를 통해 개인정보·금융 원장 데이터를 온프레미스 경계에 격리하고, 클라우드는 서비스 API와 사용자 경험을 담당하도록 역할을 분리했습니다.
...
On-Premise Gateway는 Government DB 조회의 내부 API 진입점입니다. Backend가 OCR 식별번호를 정규화한 뒤 HMAC-SHA256 해시를 생성해 Gateway에 전달하면, Gateway는 Government DB에서 해시 키로만 신원 정보를 조회합니다. 원문 주민등록번호/외국인등록번호는 HTTP 요청·응답과 DB 저장값에 포함하지 않습니다.
...
- HMAC 기반 민감정보 보호: 주민등록번호/외국인등록번호 원문 대신 정규화된 숫자값의 HMAC-SHA256 해시만 Government DB 조회 키로 사용합니다.
...
- 코드 링크: `backend/src/main/java/woorifisa/project/backend/domain/user/service/IdentityVerificationService.java`, `gateway/src/main/java/woorifisa/project/gateway/domain/foreigner/service/ForeignerService.java`
...
- 코드 링크: `backend/src/main/java/woorifisa/project/backend/domain/banking/service/BankingService.java`, `coreBanking/src/main/java/woorifisa/project/coreBanking/domain/account/service/AccountService.java`
...
- 코드 링크: `backend/src/main/java/woorifisa/project/backend/domain/banking/service/BankingService.java`, `coreBanking/src/main/java/woorifisa/project/coreBanking/domain/accountTransaction/service/AccountTransactionService.java`
...
- 코드 링크: `backend/src/main/java/woorifisa/project/backend/domain/banking/service/BankingService.java`, `coreBanking/src/main/java/woorifisa/project/coreBanking/domain/globalTransaction/service/GlobalTransactionService.java`, `coreBanking/src/main/java/woorifisa/project/coreBanking/domain/globalTransaction/service/GlobalTransactionFdsService.java`, `fds/app/main.py`
...
- 코드 링크: `ai/app/services/hospital_chat_service.py`, `ai/app/agent.py`, `backend/src/main/java/woorifisa/project/backend/domain/hospital/service/HospitalService.java`
